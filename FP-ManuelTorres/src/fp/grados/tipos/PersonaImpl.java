...
public class PersonaImpl implements Persona {
  ...
  CheckEmail(email);
	...
	public int hashCode(){
		return this.getDNI().hashCode() + this.getNombre().hashCode()*31 +
				this.getApellidos().hashCode()*31*31;
	}
			...
			if(s.equals(9)){
				...
				for(int i = 0;i<dni.length()-1;i++){
					if(!Character.isDigit(dni.charAt(i))){
						...
						throw new ExcepcionPersonaNoValida("Los 8 primeros dígitos del DNI tienen que ser números");
				
					}  else if(!Character.isLetter(dni.charAt(dni.length()-1))){
					
					throw new ExcepcionPersonaNoValida("El último dígito del DNI tiene que ser una letra ");}
				}
				...
				throw new ExcepcionPersonaNoValida("El tamaño del DNI tiene que ser 9 ");
	...
	private void CheckEmail(String correo){
		if(!(correo.isEmpty() || correo.contains("@"))){
			...
			throw new ExcepcionPersonaNoValida("Email introducido no válido");
		}
	}
